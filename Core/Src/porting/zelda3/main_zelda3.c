...
uint8_t* readSramImpl() {
  uint8_t *zelda_sram = (uint8_t *)lcd_get_inactive_buffer();
  char *pathName = "savestate/zelda3/zelda3.srm";
  fs_file_t *file;
  file = fs_open(pathName, FS_READ, FS_RAW);
  if (file != NULL) {
    fs_read(file, zelda_sram, 8192);
    fs_close(file);
    return zelda_sram;
  }
...
void writeSramImpl(uint8_t* sram) {
  char *pathName = "savestate/zelda3/zelda3.srm";
  fs_file_t *file;
  file = fs_open(pathName, FS_WRITE, FS_RAW);
